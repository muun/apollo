...
use crate::gadgets::ceil_div_usize;
    ...
    for CircuitBuilder<F, D>
{
    ...
    fn add_nonnative<FF: PrimeField>(
        ...
        self.range_check_biguint(&sum.value, &modulus);
...
pub trait WitnessNonNative<F: PrimeField64>: Witness<F> {
    fn set_non_native_target<E: PrimeField>(&mut self, target: &NonNativeTarget<E>, value: &E);
}
...
mod tests {
    ...
    #[test]
    pub fn test_nonnative_range_check() {
        const D: usize = 2;
        ...
        let unreduced_field_element: NonNativeTarget<Secp256K1Scalar> =
            builder.add_virtual_nonnative_target();

        let p_plus_one = Secp256K1Scalar::order() + BigUint::from(1u8);

        // The function set_nonnative_target reduces the biguint.
        // For testing purposes, we will set the biguint target manually.
        pw.set_biguint_target(&unreduced_field_element.value, &p_plus_one);
