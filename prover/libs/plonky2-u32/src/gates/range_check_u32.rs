...
impl<F: RichField + Extendable<D>, const D: usize> Gate<F, D> for U32RangeCheckGate<F, D> {
    ...
    fn generators(&self, row: usize, _local_constants: &[F]) -> Vec<WitnessGeneratorRef<F, D>> {
        let generator = U32RangeCheckGenerator { gate: *self, row };
        vec![WitnessGeneratorRef::new(generator.adapter())]
    }
...
mod tests {
    ...
    fn test_gate_constraint_good() {
        let mut rng = OsRng;
        let input_limbs: Vec<_> = (0..8).map(|_| rng.r#gen::<u32>() as u64).collect();
    ...
    fn test_gate_constraint_bad() {
        let mut rng = OsRng;
        let input_limbs: Vec<_> = (0..8).map(|_| rng.r#gen()).collect();
