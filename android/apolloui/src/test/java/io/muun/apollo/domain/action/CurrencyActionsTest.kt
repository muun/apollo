...
class CurrencyActionsTest : BaseTest() {
    ...
    @Mock
    private lateinit var internalMetricsProvider: InternalMetricsProvider
    ...
    fun `getLocalCurrencies in US`() {
        `when`(internalMetricsProvider.region).thenReturn(Optional.of("US"))
    ...
    fun `getLocalCurrencies in Argentina`() {
        `when`(internalMetricsProvider.region).thenReturn(Optional.of("AR"))
    ...
    fun `getLocalCurrencies locale default`() {
        `when`(internalMetricsProvider.region).thenReturn(Optional.empty())
