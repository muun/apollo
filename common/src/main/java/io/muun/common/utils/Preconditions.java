...
public final class Preconditions {
    ...
    public static int checkNotNegative(int number) {
        if (number < 0) {
            throw new IllegalArgumentException(format("negative number: " + number));
        }
        return number;
    }
     ...
     * @throws IllegalArgumentException if {@code number} is negative
     */
    public static int checkNotNegative(int number, @Nullable Object errorMessage) {
        if (number < 0) {
            throw new IllegalArgumentException(String.valueOf(errorMessage));
        }
        return number;
     ...
     * @throws IllegalArgumentException if {@code number} is negative
     */
    public static long checkNotNegative(long number) {
        if (number < 0L) {
            throw new IllegalArgumentException(format("negative number: " + number));
        }
        return number;
     ...
     * @throws IllegalArgumentException if {@code number} is negative
     */
    public static long checkNotNegative(long number, @Nullable Object errorMessage) {
        if (number < 0L) {
            throw new IllegalArgumentException(String.valueOf(errorMessage));
        }
        return number;
    ...
    static String format(String template, Object... args) {
        final String normTemplate = String.valueOf(template); // null -> "null"

        // start substituting the arguments into the '%s' placeholders
        final StringBuilder builder = new StringBuilder(normTemplate.length() + 16 * args.length);
        int templateStart = 0;
        int pos = 0;
        while (pos < args.length) {
            final int placeholderStart = normTemplate.indexOf("%s", templateStart);
