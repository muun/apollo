...
func main() {
	...
	lockMigrationsFile := lockCmd.String(
		"migrations-file",
		defaultMigrationsFile,
		"path to the migrations Go source file",
	)
	lockLockfile := lockCmd.String("lockfile", defaultLockfile, "path to the lockfile to write")

	verifyCmd := flag.NewFlagSet("verify", flag.ExitOnError)
	verifyMigrationsFile := verifyCmd.String(
		"migrations-file",
		defaultMigrationsFile,
		"path to the migrations Go source file",
	)
...
func runLock(plan []storage.Migration, migrationsFile, lockfilePath string) error {
	...
	if err != nil {
		return errors.Errorf("failed to generate lockfile: %w", err)
	}
	...
	if os.IsNotExist(err) {
		...
		return errors.Errorf("failed to read existing lockfile: %w", err)
	} else {
		...
		if err != nil {
			return errors.Errorf("failed to parse existing lockfile: %w", err)
		}
	...
	if err != nil {
		return errors.Errorf("failed to marshal lockfile: %w", err)
	}

	err = os.WriteFile(lockfilePath, data, 0644)
	if err != nil {
		return errors.Errorf("failed to write lockfile: %w", err)
	}
...
func runVerify(plan []storage.Migration, migrationsFile, lockfilePath string) error {
	generatedLockFile, err := kvmigrationlock.Generate(plan, migrationsFile)
	if err != nil {
		return errors.Errorf("failed to generate lockfile for verification: %w", err)
	}

	existingData, err := os.ReadFile(lockfilePath)
	if err != nil {
		return errors.Errorf("failed to read lockfile. Run 'lock' first: %w", err)
	}

	var committedLockFile kvmigrationlock.Lockfile
	err = json.Unmarshal(existingData, &committedLockFile)
	if err != nil {
		return errors.Errorf("failed to parse lockfile: %w", err)
	}
	...
	if len(committedLockFile.Migrations) < len(generatedLockFile.Migrations) {
		return errors.Errorf("lockfile has %d migrations but plan has %d, run 'lock' to update",
...
func failIfHistoryIsModified(
	committedLockfile kvmigrationlock.Lockfile,
	generatedLockfile *kvmigrationlock.Lockfile,
) error {
	if len(committedLockfile.Migrations) > len(generatedLockfile.Migrations) {
		return errors.Errorf(
			"plan has %d migrations but lockfile has %d: deleting migrations is not allowed",
			len(generatedLockfile.Migrations),
			len(committedLockfile.Migrations),
		)
	}
	for i, existing := range committedLockfile.Migrations {
		if existing.Hash != generatedLockfile.Migrations[i].Hash {
			return errors.Errorf("migration %d (%q) was modified", i+1, existing.Description)
		}
	}
	return nil
}
