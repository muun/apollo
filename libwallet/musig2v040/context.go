...
var (
	...
	ErrSignersNotSpecified = errors.Errorf("total number of signers or all " +
	...
	ErrSignerNotInKeySet = errors.Errorf("signing key is not found in key" +
		" set")

	// ErrAlredyHaveAllNonces is called when RegisterPubNonce is called too
	// many times for a given signing session.
	ErrAlredyHaveAllNonces = errors.Errorf("already have all nonces")
	...
	// known.
	ErrNotEnoughSigners = errors.Errorf("not enough signers")
	...
	ErrCombinedNonceUnavailable = errors.Errorf("missing combined nonce")

	// ErrTaprootInternalKeyUnavailable is returned when a user attempts to
	// obtain the
	ErrTaprootInternalKeyUnavailable = errors.Errorf("taproot tweak not used")

	// ErrNotEnoughSigners is returned if a caller attempts to obtain an
	// early nonce when it wasn't specified
	ErrNoEarlyNonce = errors.Errorf("no early nonce available")
)
...
func (c *Context) EarlySessionNonce() (*musig2.Nonces, error) {
...
func WithPreGeneratedNonce(nonce *musig2.Nonces) SessionOption {
...
func (c *Context) NewSession(options ...SessionOption) (*Session, error) {
	...
	if localNonces == nil {
		return nil, errors.New("a pre-created nonce must be passed onto the session")
	}
	...
	s := &Session{
		...
		pubNonces:   make([][musig2.PubNonceSize]byte, 0, c.opts.numSigners),
...
// public nonce.
func (s *Session) PublicNonce() [musig2.PubNonceSize]byte {
...
// accounted for.
func (s *Session) RegisterPubNonce(nonce [musig2.PubNonceSize]byte) (bool, error) {
